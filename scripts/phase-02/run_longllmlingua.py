"""
LongLLMLingua (question-aware compressor) benchmark on ZeroSCROLLS.

Pipeline per case:
  1. LongLLMLingua compress(context, question, target_token) — preserves answer-bearing
     sentences, ideal for RAG (Track 2 lever C in master plan).
  2. Gemini call (flash-lite by default, configurable).
  3. LLM-as-judge evaluation.

CLI flags (all optional, defaults work for a quick run):
  --n INT            cases to run (default 20)
  --seed INT         sample seed (default 42)
  --target-token INT compression target (default 1500)
  --model STR        Gemini model id (default gemini-3.5-flash-lite)
  --judge-profile STR  judge LLM profile (default nim; or openrouter / auto-chain)
  --tasks STR        comma-separated subset, default all 9 (narrative_qa excluded)
  --sleep FLOAT      seconds between Gemini calls (default 3.0)
  --max-chars INT    truncate context beyond this BEFORE compression
  --out PATH         output JSONL
  --summary-out PATH output summary JSON
  --no-judge         skip judge step (raw calls only)

Usage:
  conda activate vsf
  python scripts/phase-02/run_longllmlingua.py --n 20
  python scripts/phase-02/run_longllmlingua.py --n 50 --judge-profile openrouter
"""
...
JUDGED_PATH = REPO_ROOT / "results" / "phase-02-baseline-200-en-judged.jsonl"
DEFAULT_OUT = REPO_ROOT / "results" / "phase-02-longllmlingua.jsonl"
DEFAULT_SUMMARY = REPO_ROOT / "results" / "phase-02-longllmlingua-summary.json"
...
def get_longllmlingua():
    """Lazy-init LongLLMLingua compressor (Microsoft llmlingua)."""
    global _longllmlingua
    if _longllmlingua is None:
        from llmlingua import PromptCompressor
        _longllmlingua = PromptCompressor(
            model_name="microsoft/llmlingua-2-xlm-roberta-large-meetingbank",
            use_llmlingua2=True,
            device_map="cpu",
        )
    return _longllmlingua


def compress_longllmlingua(context: str, question: str,
                          target_token: int = 1500) -> dict:
    """Run LongLLMLingua on (context, question). Returns dict with stats."""
    t0 = time.perf_counter()
    try:
        pc = get_longllmlingua()
        # LongLLMLingua signature: context, target_token, context_budget_ratio,
        # device, question for question-aware mode.
...
# Judge
# ---------------------------------------------------------------
def judge_case(question: str, gold: str, pred: str, profile: str | None,
               log_path: Path | None) -> dict:
    """LLM-as-judge via unified LLMJudge."""
...
# ---------------------------------------------------------------
# Sample loader (re-uses baseline sampling logic for paired comparison)
# ---------------------------------------------------------------
def load_stratified_sample(n: int, seed: int, tasks: list[str] | None) -> list[dict]:
    """Load stratified sample from existing zero_scrolls_200.jsonl OR rebuild
    a fresh sample with same seed if not available.
    """
    if SAMPLE_PATH.exists():
        ...
        # Fallback: build a fresh sample from cached ZeroSCROLLS
        from scripts.phase_01._common import stream_zero_scrolls, detect_fields
        all_rows = []
        for r in stream_zero_scrolls():
            if tasks and r.get("task") not in tasks:
                continue
            fields = detect_fields(r)
            all_rows.append({"row": r, "task": r["task"], "fields": fields})

        by_task = defaultdict(list)
        for c in all_rows:
            by_task[c["task"]].append(c)
        rng = random.Random(seed)
        quotas = {t: max(1, round(n * len(v) / len(all_rows)))
                  for t, v in by_task.items()}
        diff = n - sum(quotas.values())
        if diff > 0:
            for t in sorted(quotas, key=lambda x: -quotas[x]):
...
def main() -> int:
    ...
    p.add_argument("--target-token", type=int, default=1500,
                   help="LongLLMLingua target token after compression")
    p.add_argument("--model", default=DEFAULT_MODEL,
                   help=f"Gemini model id (default {DEFAULT_MODEL})")
    p.add_argument("--judge-profile", default="nim",
                   help="LLMJudge profile (nim | openrouter | gemini | auto)")
    p.add_argument("--tasks", default="",
                   help="Comma-separated task subset; empty = all 9 default")
    p.add_argument("--sleep", type=float, default=3.0)
    p.add_argument("--max-chars", type=int, default=0,
                   help="If >0, truncate context before compression")
    p.add_argument("--out", type=Path, default=DEFAULT_OUT)
    p.add_argument("--summary-out", type=Path, default=DEFAULT_SUMMARY)
    p.add_argument("--ops-out", type=Path, default=None,
                   help="Optional ops log JSONL for LLMJudge")
    p.add_argument("--no-judge", action="store_true",
                   help="Skip LLM-as-judge step (raw calls only)")
    args = p.parse_args()

    tasks = [t.strip() for t in args.tasks.split(",") if t.strip()] or None
    judge_log = args.ops_out

    print(f"=== LongLLMLingua benchmark on {args.n} cases ===")
    print(f"  Model:   {args.model}")
    print(f"  Target:  {args.target_token} tokens")
    ...
    print("[2/4] Running LongLLMLingua + Gemini...")
    all_records = []
    ...
    for i, case in enumerate(sample):
        ...
        if args.max_chars > 0 and len(context) > args.max_chars:
            context = context[: args.max_chars]

        # 1. Compress
        comp = compress_longllmlingua(context, question, args.target_token)
        if comp["status"] != "ok":
        ...
        if args.no_judge:
            ...
            jj = judge_case(question, gold, llm["text"], args.judge_profile, judge_log)
        ...
        record = {
            ...
            "mode": "longllmlingua",
            # Compression
            "origin_tokens": int(comp["origin_tokens"]),
            "compressed_tokens": int(comp["compressed_tokens"]),
        ...
        print(f"  [{i+1:>3}/{len(sample)}] {marker} {case_id:<32} task={task:<14} "
              f"comp={comp['compressed_tokens']:>4}tok({comp['ratio']:.1f}x) "
              f"in={record['input_tokens']} out={record['output_tokens']} "
              f"save={record.get('token_saving', 0) or 0:.1%} "
              f"ms={float(comp['compress_ms']) + float(llm['latency_ms']):.0f}")
    ...
    total_ms = [r["compress_ms"] + r["llm_latency_ms"] for r in ok_records]
    ...
    ratios = [r["compress_ratio"] for r in ok_records]
    ...
    print(f"  {'Task':<18} {'Acc':>8} {'CompTokMed':>11} {'RatioMed':>9} {'InTokMed':>9}")
    print("  " + "-" * 60)
    per_task = defaultdict(lambda: {"n": 0, "correct": 0, "comp": [], "ratio": [], "in": []})
    for r in ok_records:
        t = r["task"]
        per_task[t]["n"] += 1
        if r["judge_correct"]:
            per_task[t]["correct"] += 1
        per_task[t]["comp"].append(r["compressed_tokens"])
    ...
    for t in sorted(per_task):
        ...
        print(f"  {t:<18} {acc:>8} {cmed:>11} {rmed:>9} {imed:>9}")
