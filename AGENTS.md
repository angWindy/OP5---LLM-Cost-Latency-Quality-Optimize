...
## Codebase-first rule (added 2026-09-22)

> User quy định: **ưu tiên dùng codebase** (scripts/ + src/op5/) thay vì viết code ad-hoc trên terminal.

- **Mọi thay đổi có ý nghĩa** (logic, threshold, prompt, schema) **phải vào codebase** dưới dạng:
  - New/edited script: `scripts/phase-XX/<name>.py`
  - New/edited module: `src/op5/<pkg>/<name>.py`
  - Config / profile: `src/op5/llm/profiles/*.yaml`
  - Kèm worklog entry giải thích lý do.
- **Terminal `python3 -c '...'` / inline script chỉ dùng cho**:
  - Quick smoke-test API key / connectivity (1-2 lần gọi).
  - Inspect / debug một record từ JSONL output (read-only).
  - Verify một config value trước khi commit.
  - **KHÔNG dùng** `python3 -c` để chạy experiment, process batch, hay gọi LLM nhiều lần.
- Khi reproduce một experiment: luôn invoke script qua `conda activate vsf && python scripts/phase-XX/<name>.py`,
  không inline lại logic trên terminal.
- Trước khi `python3 -c` test, hỏi: "Có cách nào dùng script có sẵn trong `scripts/` không?" — nếu có thì dùng cái đó.
